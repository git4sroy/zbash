...
[![Build Status](https://travis-ci.org/zbash/zbash.svg?branch=master)](https://travis-ci.org/zbash/zbash) [![Join the chat at https://gitter.im/zbash/zbash](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/zbash/zbash?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)
...
1. Check out a clone of this repo to a location of your choice, such as: `git clone --depth=1 https://github.com/zbash/zbash.git ~/.ZBASH`
2. Run `~/.ZBASH/install.sh` (it automatically backs up your `~/.bash_profile` or `~/.bashrc`, depending on your OS)
...
```
zbash show aliases        # shows installed and available aliases
zbash show completions    # shows installed and available completions
zbash show plugins        # shows installed and available plugins
zbash help aliases        # shows help for installed aliases
zbash help completions    # shows help for installed completions
zbash help plugins        # shows help for installed plugins
```
...
* `aliases/custom.aliases.zbash`
* `completion/custom.completion.zbash`
* `lib/custom.zbash`
* `plugins/custom.plugins.zbash`
* `custom/themes/<custom theme name>/<custom theme name>.theme.zbash`

Anything in the custom directory will be ignored, with the exception of `custom/example.zbash`.
...
You can see the theme screenshots [here](https://github.com/zbash/zbash/wiki/Themes)

Alternatively, you can preview the themes in your own shell using `BASH_PREVIEW=true reload`

## Uninstalling

To uninstall Bash it, run the `uninstall.sh` script found in the `$ZBASH` directory:
...
This will restore your previous Bash profile. After the uninstall script finishes, remove the Bash it directory from your machine (`rm -rf $ZBASH`) and start a new shell.
...
This behavior can be overridden with the `ZBASH_LEGACY_PASS` flag as follows:

Set `ZBASH_LEGACY_PASS` to 'true' to force Bash it to always **create** the `pass` alias to `passgen`:

* `export ZBASH_LEGACY_PASS=true`

Unset `ZBASH_LEGACY_PASS` to have Bash it **return to default behavior**:

* `unset ZBASH_LEGACY_PASS`
...
zbash uses the following variables to set the shell's proxy settings when you call `enable-proxy`.
These variables are best defined in a custom script in zbash's custom script folder (`$ZBASH/custom`), e.g. `$ZBASH/custom/proxy.env.zbash`
* `ZBASH_HTTP_PROXY` and `ZBASH_HTTPS_PROXY`: Define the proxy URL to be used, e.g. 'http://localhost:1234'
* `ZBASH_NO_PROXY`: A comma-separated list of proxy exclusions, e.g. `127.0.0.1,localhost`
...
[contribute]: https://github.com/zbash/zbash/contributors
